...
def main() -> int: 
    ...
    if not args.Folder:
        print("Please include the folder where the unzipped project files are contained. \n Use the -o flag with \" /path/to/file \".")
    
    try: 
        folder_path = os.path.abspath(args.Folder)
        correct_path = input(f"\nIs this is the correct path: {folder_path}? [y/n]")
        if correct_path != 'y':
            print("No worries. Run the script again with the correct path. Or go bother Kevin if the path continues to fail.")
            return -1  
    except Exception as e: 
        print(f"File Path {args.Folder} does not exists. Please correct the pathing. The path can be a relative path.")
        return -1 
    ...
    try: 
        ...
        if correct_files != 'y':
            ...
            if ds_store == 'y':
                ...
                correct_files = input("Are these the correct files? [y/n]")
                if correct_files != "y":
                   print("No worries... I'm aborting the operation and you can try again. Or go repremand Kevin. Your choice!") 
            elif ds_store != 'y':
                print("No worries... I'm aborting the operation and you can try again. Or go repremand Kevin. Your choice!")
            return -1
    except Exception as e: 
        print(f"Error: {e}")
        print("Something went wrong with getting all the files in the folder. If this continues to happen go yell at Kevin for his mistakes")
        return -1 


    permission = input("Do I have permission to make two new folders while the merging occurs? You can delete them afterwards. [y/n]")
    if permission != 'y':
        print("No worries at all. Talk with Mia or Kevin to see what can be done about this.") 
        return -1
    ...
    try: 
        ...
        print(f"Error: {e}")
        print("Something went wrong with creating the directory. I'll print the error, but usually the error is due to there already being the directories temp and output in the root")
    ...
    try:
        ...
        print("An error occured while copying the the data from the original folder. Talk with Kevin to get this resolved")
        return -1
    ...
    while output_len != 1: 
        ...
        for i in range(len(os.listdir("./temp"))):
            ...
            if i == j: 
                ...
                shutil.copytree(f"./temp/{temp_files[i]}", f"./output/output_project_{i}", dirs_exist_ok=True)
            elif i < j: 
                ...
                subprocess.run(npm_command.split(), stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            j -= 1
        output_len = len(os.listdir("./output"))
